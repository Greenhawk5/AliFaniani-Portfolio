...
/**
 * The exact Archimedean spiral path from the "Volute" loader
 * (circleloaders.dominikakissi.com #21). The same path is drawn twice: a faint
 * ghost, and an animated dash that draws/erases itself inside a slowly
 * rotating rig. Scaled via viewBox — no extra assets, pure inline SVG + CSS.
 */
const VOLUTE_PATH =
...
function Volute() {
  ...
  const isStatic = motionMode === 'reduced'

  return (
    <svg
      className="vol text-accent drop-shadow-[0_0_6px_rgba(57,255,139,0.5)]"
      viewBox="0 0 64 64"
      width={72}
      height={72}
      fill="none"
    ...
    >
      <style>{`
        ...
        .vol-rig {
          ...
          animation: vol-turn calc(var(--dur) * 2.4) linear infinite;
        }
        .vol-line {
          stroke-width: 1.9;
          stroke-dasharray: 100 100;
          stroke-dashoffset: 100;
          animation: vol-draw var(--dur) infinite;
        }
        @keyframes vol-draw {
        ...
        .vol[data-static] .vol-rig, .vol[data-static] .vol-line { animation: none; }
        .vol[data-static] .vol-line { stroke-dashoffset: 0; }
...
export function LoadingVeil() {
  ...
  // Short minimum keeps the existing exit transition smooth; readiness itself
  // is driven purely by real Home asset loading (see HomeReadiness in Scene).
  ...
  const hidden = minElapsed && sceneReady
